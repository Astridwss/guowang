...
### 视觉/大语言模型（Qwen2-VL-7B-Instruct）部署命令：
...
CUDA_VISIBLE_DEVICES=1 nohup python -m vllm.entrypoints.openai.api_server   
	...
	--port 8002   
	--max-model-len 4096   
	--enforce-eager > /data/home/wangshan/guowang_LLM/qwen_8002.log 2>&1 &
```

### Embedding 向量模型（bge-large-zh-v1.5）部署命令：
...
```
运行脚本python llm_clients/asr_client.py
```

## 参数说明

```
CUDA_VISIBLE_DEVICES=1：显式指定只将模型加载到编号为 1 的那张显卡上，做到多卡环境的算力隔离。
--model /data/home/...：就是这里！直接塞进去绝对路径，卡死本地加载。
--max-model-len 4096：对于 Qwen2-VL 这种极度吃显存的多模态模型，把长度限制在 4096 是非常明智的防 OOM（显存溢出）手段。
--enforce-eager：强制使用 Eager 模式执行，不采用 PyTorch 默认的 CUDAGraph 图优化。这个参数对于多模态模型或者显存紧凑的生产环境来说，可以显著稳定显存占用，避免动态输入导致图重建失败。
```
...
2、启动命令：bash star.sh    或 cd ./api/ && python -m uvicorn main:app
3、构建镜像：docker build -t 镜像名:版本 .
...
4、打包镜像：docker save -o 压缩包名.tar 镜像名:版本
5、配合后端需要挂载nfs,部署/启动容器：docker run -d -it -p 宿主机端口:8080 -v 宿主机路径:容器里路径 --name 容器名 镜像名:版本 /bin/bash
	例子：
		docker run -d -p 7511:8080 \
		-v /data/jfs/grid/:/data/jfs/grid/ \
		--name grid-algo-amd \
		grid-algo-amd:v2.3

6、进入容器后调试启动命令：python -m uvicorn main:app --host 0.0.0.0 --port 8080

7、API的docs文档查看：部署ip:端口/docs，例如192.168.0.80:7511/docx

8、AMD镜像：./grid-algo-amd.tar
9、ARM镜像：./grid-algo-arm.tar
```
